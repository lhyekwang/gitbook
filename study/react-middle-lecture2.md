...
컴포넌트의 함수의 반환값
--
리액트요소반환, 문자열, 숫자, div 리액트요소, 배열, 배열 요소로반환 (배열로 반환시 key 요소가 없으면 에러), fragment(div를 안쓰고 싶을때 , <><> 이렇게 쓰기도 함.), null, boolean 값, 리액트 portal (root 말고 다른곳에 렌덜이가능 / ReactDom 에 있음 (모달만들때 사용) 
```
...
리액트 요소는 type 은 컴포넌트 함수가 입력되어있음. 저 함수를 이용해서 렌더링을 위한 정보를 얻는다.   
리액트 요소는 변경하려고 하면 에러가 발생된다. 콘솔창보면 변경된것만 렌더링되고있음. div 요소에 key가 변경되면 다른요소라고 생각해서 새로만들어 붙임. style 은 다른요소라고 생각하지 않음   
component 의 키를 변경하게 되도, ummount와 mount를 하면서 새로 그려진다.   
...
>render 단계에서 그리고, memo 단계에서 다른부분만 값을 바꾼다. 이
...
컴포넌트 렌더리중에 부수효과를 발생시키는건 거의 없고, concurrent mode로 동작할 미래의 리액트는 컴포넌트 함수가 호출되도 중간에 렌더링을 취소될 수 있음.  
